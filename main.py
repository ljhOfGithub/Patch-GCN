...
def main(args):
    ...
    for i in folds:
        ...
        #datasets.dataset_survival.Generic_MIL_Survival_Dataset object
        ...
        if args.task_type == 'survival':
            # import pdb; # pdb.set_trace()
            #datasets.dataset_survival.Generic_Split object
...
# parser.add_argument('--data_root_dir', type=str, default='~/data/mntdata/data0/LI_jihao/BRCA_fea/h5_files', help='data directory')
# parser.add_argument('--data_root_dir', type=str, default='/home/jupyter-ljh/data/mntdata/data0/LI_jihao/BRCA_fea/h5_files', help='data directory')
...
# parser.add_argument('--results_dir',     type=str, default='./results', help='Results directory (Default: ./results)') #存放结果的主目录
parser.add_argument('--results_dir',     type=str, default='./results_bug', help='Results directory (Default: ./results)') #存放结果的主目录 方便调试
parser.add_argument('--which_splits',    type=str, default='5foldcv', help='Which splits folder to use in ./splits/ (Default: ./splits/5foldcv') #存放结果的次目录
...
parser.add_argument('--split_dir',       type=str, default='tcga_brca', help='Which cancer type within ./splits/<which_splits> to use for training. Used synonymously for "task" (Default: tcga_blca_100)') #存放结果的次目录
...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['deepset', 'amil', 'mi_fcn', 'dgc', 'patchgcn'], default='patchgcn', help='Type of model (Default: mcat)')
...
parser.add_argument('--gc',              type=int, default=32, help='Gradient Accumulation Step.') #梯度累积的步长（Gradient Accumulation Step）
parser.add_argument('--max_epochs',      type=int, default=20, help='Maximum number of epochs to train (default: 20)')
# parser.add_argument('--max_epochs',      type=int, default=2, help='Maximum number of epochs to train (default: 20)') #方便调试
...
# parser.add_argument('--early_stopping',  action='store_true', default=False, help='Enable early stopping') #早停
parser.add_argument('--early_stopping',  action='store_true', default=True, help='Enable early stopping') #早停 方便调试
...
if 'survival' in args.task:
    ...
    dataset = Generic_MIL_Survival_Dataset(
        ...
        csv_path = '/home/jupyter-ljh/data/mydata/Patch-GCN-master/dataset_csv/tcga_brca_all_clean.csv',
                                           mode = args.mode,
...
# args.results_dir = '~/data/mydata/Patch-GCN-master/myresult'
if not os.path.isdir(args.results_dir):
